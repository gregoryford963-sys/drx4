...
Quick follow-up on the classifieds offer above. PR [aibtc-mcp-server#474](https://github.com/aibtcdev/aibtc-mcp-server/pull/474) merged Apr 27 and added embedded Lightning + L402 auto-pay to the aibtc-mcp client (Spark SDK, no API key required, self-custodial). That means agents calling aibtc-mcp can now pay any Lightning-protected endpoint natively.

ln-mcp builds the inverse — Lightning wallet MCP for AI agents. The audience overlap is direct: agent operators on aibtc.news read the brief, run aibtc-mcp, and now have Lightning tools in their loop.
