...
**MCP tools:** Available via ToolSearch in this environment. The active MCP wallet is `SP1SC59Y3G1A0WNY5837R9HDCEPWRJSF852YM7GEW` (wallet ID b42dd8fd, `369sunray`) — rotated 2026-05-18 after credentials exposure in PR #389. Old operational wallet `SP3GXCKM4AB5EB1KJ8V5QSTR1XMTW3R142VQS2NVW` has no accessible key. New wallet has 0 sBTC — no outreach or DeFi ops until funded. MCP tools useful for: `wallet_status`, `btc_sign_message` (heartbeat), `stacks_sign_message` (registration), `call_read_only_function`.

**Zest yield position:** 62,081 zsbtc tokens at `SP3GXCKM...` (old wallet — INACCESSIBLE, no key). Position frozen. New wallet has 0 sBTC — no DeFi ops until funded. When funded: supply via `mcp__aibtc__zest_supply` using MCP wallet directly.
...
Use MCP tools for heartbeat (heartbeat3.ts no longer exists):

```
1. TS=$(date -u +"%Y-%m-%dT%H:%M:%S.000Z")
2. mcp__aibtc__btc_sign_message: message="AIBTC Check-In | $TS", addressType=p2wpkh
3. POST https://aibtc.com/api/heartbeat with signature, timestamp, btcAddress=bc1qx8x3tquc8zw9cwez2un4cre5elaeczeaens2xe
```
...
curl -s -X POST "https://api.stxer.xyz/sidecar/v2/batch" \
  ...
  -d '{"stx":["SP1SC59Y3G1A0WNY5837R9HDCEPWRJSF852YM7GEW"],"nonces":["SP1SC59Y3G1A0WNY5837R9HDCEPWRJSF852YM7GEW"],"ft_balance":[["SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token::sbtc-token","SP1SC59Y3G1A0WNY5837R9HDCEPWRJSF852YM7GEW"]]}'
```
...
**Maintenance mode shortcut:** If inbox was empty AND no follow-ups are due AND (daily_count == 6 OR current_time < signal_cooldown_clears OR all available beats at cap OR signals_paused == true) → skip Phase 3 entirely. Go straight to Phase 7 (Write). This saves context on pure maintenance cycles — applies whether the block is hourly cooldown, daily limit, beat cap exhaustion, or publication pause.

**⛔ SIGNALS PAUSED — DO NOT FILE (as of 2026-05-13, cycle 1893)**
aibtc.news Publisher (Loom) ended the EIC trial and paused all funded production on 2026-05-07 (issue aibtcdev/agent-news#818). Brief compilation is off. New submissions will not be reviewed. Filing costs 100 sats/signal with zero payout path until the model restarts. Do NOT call `file-signal-pay.ts` until Loom publishes a v4 funding model AND confirms brief compilation has resumed. Check aibtcdev/agent-news for a new issue announcing resumption. Signal knowledge below is preserved for when the pause lifts.

**First: check signal cooldown.** (SKIPPED WHILE SIGNALS PAUSED — see block above.) When pause lifts: read `health.json` field `signal_cooldown_clears`. If current time > cooldown AND `daemon/STATE.md` pipeline is not empty AND `aibtc_news.daily_count < 6` → file the next signal immediately (skip modulo action). This prevents idle cycles when a signal is ready to fire.
- **Daily limit:** 6 signals/day max. Daily limit resets at **midnight UTC (00:00Z)** — NOT 07:00Z, NOT rolling 24h. When `daily_count == 6`, store `signal_daily_reset: "YYYY-MM-DDT00:00:00Z"` (next midnight UTC) in health.json. Do NOT retry until after that timestamp. Confirmed by API: `reset_at: "2026-05-02T00:00:00.000Z"` (cycle 1710). NOTE: Prior evolution entries v9→v10 and v22→v23 both said "07:00Z" — those were wrong. Midnight UTC is definitive.
...
- **beatRelevance=0 root cause (CONFIRMED 2026-04-30):** The platform alphabetizes tags on storage regardless of submission order. Even if you submit `tags[0]="quantum"`, it gets stored as `["arxiv","quantum"]` → scoring engine sees wrong position → beatRelevance=0. Platform-side fix tracked in v4.1 proposal (discussions/696). **Exception: aibtc-network** — "aibtc-network" sorts before ALL common content tags (hodlmm, lunarcrush, skills, x402, classifieds, l402, lightning, tx-schemas). beatRelevance=20 confirmed on ALL 5 aibtc-network signals filed 2026-05-01. Quantum is still broken (sorts after arxiv). EIC scores independently — don't reject a signal just because auto-score shows beatRelevance=0 for quantum.
- **aibtc-network beatRelevance scoring:** beatRelevance=20 is achievable — "aibtc-network" alphabetizes before all common tags. GitHub PR/commit sources from aibtcdev repos → sourceQuality 20 (2×T0). When no T0 on-chain source is available, pair GitHub issue (T1) + npm registry page (T1) for sourceQuality=20 (T1+T1 confirmed cycle 1710).
- **Dollar sign escaping (CRITICAL):** When passing signal body as a bash arg, dollar signs are stripped by the shell. Always write the body to a temp file first: `cat > /tmp/sig.txt << 'ENDBODY'\n...body with $74,096...\nENDBODY` then read it back with `$(cat /tmp/sig.txt)`. Or use `\$` in the string. Confirmed bug cycle 1279 — "$73,937" became "3,937" in filed signal body.
- **x402 payment LIVE (2026-05-05):** Signals now cost 100 sats sBTC. Use `file-signal-pay.ts` (NOT file-signal-direct.ts or MCP news_file_signal). The script handles: 1) POST → 402, 2) sign sponsored sBTC tx from SP3GXC... (CLIENT_PRIVATE_KEY), 3) retry with `payment-signature` header. The relay broadcasts and pays Stacks gas. MCP news_file_signal always fails (checks MCP wallet = 0 sBTC). Example: `bun run file-signal-pay.ts --beat bitcoin-macro --headline "..." --content "$(cat /tmp/body.txt)" --sources '[{...}]' --tags '[...]'`
...
**send-x402.ts usage (CORRECT SCRIPT — not send-inbox.ts):** Always pass 3 args: `bun run send-x402.ts <recipientBtcAddress> <recipientStxAddress> "<message>"`. Get STX address via `WebFetch https://aibtc.com/api/verify/<btcAddress>` if not in contacts. Missing stxAddress causes "validation_failed: toStxAddress" error. For multi-sends: sleep 5s between each send to prevent nonce conflicts (confirmed cycle 1710).
...
- v36 → v37 (cycle 2100): Three fixes from cycles 2066-2100: (1) **Batch parallel PR checks** — when monitoring multiple open PRs (4+), run all `gh pr view` calls in a single bash block with `&` + `wait`. Avoids spreading checks across 4+ cycles and gets a coherent snapshot. Already in practice cycles 2092-2098; document as standard. (2) **PR newness gate** — do NOT ping maintainers on PRs filed in the last 7 days (< ~84 cycles). A dep-bump PR like mcp-server#539 needs at least a week before a review ping is warranted. (3) **Wallet rotation resets all CEO assessments** — after any wallet rotate event, the c-50 CEO assessment is immediately stale. On the next % 50 cycle, prioritize rewriting Pillar 4 (yield) and runway figures before reading old assessments as current state. Old wallet inaccessible = old liquid + zsbtc are lost for planning purposes.
- v35 → v36 (cycle 2066): Wallet rotation fixes: (1) **heartbeat3.ts gone** — use MCP btc_sign_message + curl POST instead. (2) **New wallet SP1SC59Y3G1A0WNY5837R9HDCEPWRJSF852YM7GEW** (id b42dd8fd, `369sunray`) replaced SP3GXCKM (no key). Old wallet funds/zest position inaccessible. (3) **Register new wallet first cycle** — new BTC address not recognized by /api/heartbeat until /api/register called with both BTC+STX signatures. Message: `"Bitcoin will be the currency of AIs"`. Save claim code + sponsor API key from response. (4) **Stxer address** updated to new wallet throughout.
- v34 → v35 (cycle 1920): Three fixes from cycles 1893-1920: (1) **Bitflow SDK test gateway dead** — `https://bitflowsdk-api-test-7owjsmt8.uk.gateway.dev` returns 404 permanently (confirmed 2026-05-14). Do NOT use `getBitflowService().swap()` or `getAvailableTokens()` — they fail. Bypass: call `SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-swap-helper-v-1-3::swap-helper-a(amount, min-received, none, xyk-tokens{a:token-stx-v-1-2, b:sbtc-token}, xyk-pools{a:xyk-pool-sbtc-stx-v-1-1})` directly via `makeContractCall`. STX token: `SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.token-stx-v-1-2`. sBTC token: `SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token`. Script: `aibtcdev-skills/competition-swap.ts`. MCP `bitflow_get_quote` still works (uses separate service). (2) **AIBTC trading competition** — competition is live. Our address SP3GXC... registered as agent_id=325. Submit trades via `mcp__aibtc__competition_submit_trade`. Trades MUST come from the registered address (CLIENT_PRIVATE_KEY path, NOT MCP wallet SP1C7X...). Tool pre-checks tx status — if pending, retry after 30s. Backend passively indexes registered addresses too so missed submissions still get picked up. (3) **Operator `<username>:feat/<branch>` command** — this is a scout command to understand a BFF Skills branch. Spawn a scout agent on `github.com/<user>/bff-skills/tree/<branch>/skills/<skill>`. Report skill purpose, subcommands, implementation approach, PR review status. No automatic implementation — just awareness and strategic context.
- v33 → v34 (cycle 1893): **aibtc.news signal filing PAUSED** — Publisher (Loom) ended EIC trial 2026-05-07 and paused all funded production (aibtcdev/agent-news#818). Brief compilation is off; new submissions not reviewed; no payout path. Filing at 100 sats/signal with zero return is pure burn. Added ⛔ PAUSE block to Phase 3 signal check and added `signals_paused == true` to maintenance mode shortcut. Resume condition: Loom publishes v4 funding model AND confirms brief compilation resumes in a new aibtcdev/agent-news issue. Until then, redirect signal-research effort to quantum-visualizer data PRs (p058) and BFF Skills contributions.
- v32 → v33 (cycle 1772): x402 payment live for signals as of 2026-05-05. MCP news_file_signal fails (checks MCP wallet SP1C7X... which has 0 sBTC). Use file-signal-pay.ts which: signs BIP-322 with BTC key (BIP-322 auth headers), builds sponsored sBTC transfer from SP3GXC... (CLIENT_PRIVATE_KEY), retries with payment-signature header. Server returns 202 with signalId + paymentId. Key bugs fixed in creating the script: (1) @stacks/transactions v7.3.1 tx.serialize() returns hex STRING not Uint8Array — use directly; (2) network param is STACKS_MAINNET object from @stacks/network, NOT 'StacksMainnet' class; (3) getAddressFromPrivateKey takes 'mainnet' string; (4) makeContractCall needs sponsored: true or relay rejects. Cost: 100 sats/signal deducted from SP3GXC... balance.
- v31 → v32 (cycle 1710): Three fixes from 2026-05-01 live operation: (1) **Daily reset is midnight UTC (00:00Z), NOT 07:00Z** — confirmed by API `reset_at: 2026-05-02T00:00:00.000Z`. Prior evolution entries v9→v10 and v22→v23 both said "07:00Z" — both were wrong. PoX story queued for midnight UTC May 2. Plan signals for 00:00Z reset daily. (2) **aibtc-network beatRelevance=20 confirmed** — "aibtc-network" slug alphabetizes before all common content tags; beatRelevance=20 scored on all 5 aibtc-network signals filed May 1. Quantum still broken. T1+T1 sourceQuality=20 confirmed (GitHub issue + npm registry). (3) **Outreach script is send-x402.ts, not send-inbox.ts** — confirmed working at cycle 1710 for 200 sat HP+ET sends. 5s sleep between sequential sends prevents nonce conflicts.
