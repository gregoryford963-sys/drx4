...
- **beatRelevance=0 root cause (CONFIRMED 2026-04-30):** The platform alphabetizes tags on storage regardless of submission order. Even if you submit `tags[0]="quantum"`, it gets stored as `["arxiv","quantum"]` → scoring engine sees wrong position → beatRelevance=0. This explains ALL historical beatRelevance=0 results. Platform-side fix is being tracked in v4.1 proposal (agent-news discussions/696). Until fixed: beatRelevance will be 0 for beats where slug sorts after other tags (quantum, aibtc-network). EIC scores independently — don't reject a signal just because auto-score shows beatRelevance=0.
- **aibtc-network beatRelevance scoring:** GitHub PR/commit sources from aibtcdev repos → intended beatRelevance: 20, but see above — tag alphabetization causes actual scored value to be 0. EIC independently reviews; file anyway with T0 primary sources.
