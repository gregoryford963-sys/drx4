#!/usr/bin/env bash
# May 1 PT fire queue — 3 v3 pitches (mcp-sophon / attestify-os / lemonade-bitcoin)
# Pre-flighted cycle 2034px; lint 0/0; canonical pipeline updated 97→100 cycle 2034py.
      ...
      if .id == $id then
        . + {
          ...
          touches: (.touches + [{
            ...
            direction: "outbound",
            url: $url,
            summary: "May 1 PT v3 pitch fire (cycle 2034px pre-flight). 3k sats / 7d, measurement in progress."
          }])
        } |
...
fire_one "p097" "lacausecrypto/mcp-sophon" "$DRAFTS/p097-mcp-sophon.md" \
  "May 1 PT direct first-touch — deterministic context compressor for MCP agents, +24% tokens / +49% $ saved on top of caching, 5⭐ MIT"
sleep 1
fire_one "p098" "attestifyagent/attestify-os" "$DRAFTS/p098-attestify-os.md" \
  "May 1 PT direct first-touch — unified x402 Agent OS w/ memory + cost control + sandbox + orchestration, LangGraph/CrewAI ready, fresh 1d-old"
sleep 1
fire_one "p099" "Bortlesboat/lemonade-bitcoin-agent" "$DRAFTS/p099-lemonade-bitcoin.md" \
  "May 1 PT direct first-touch — local Bitcoin AI agent on AMD Lemonade + Satoshi API, privacy-first audience match, 4⭐"

echo ""
echo "=== May 1 PT fire complete (proofs + pipeline atomic) ==="
